...
def _run_siftd(*args: str, stdin: bytes | None = None) -> subprocess.CompletedProcess:
    cmd = ["uv", "run", "siftd", *args]
    return subprocess.run(cmd, input=stdin, capture_output=True, timeout=30)
...
class TestSendReceivePipeline:
    ...
    def test_send_all_receive(self, tmp_path):
        source = _seed_db(tmp_path)
        target = tmp_path / "target.db"

        send = _run_siftd("--db", str(source), "db", "send")
        ...
        receive = _run_siftd("--db", str(target), "db", "receive", stdin=send.stdout)
        assert receive.returncode == 0

        assert _count_conversations(target) == 3
        assert _external_ids(target) == {"a", "b", "c"}

    def test_send_with_tag_filter(self, tmp_path):
        source = _seed_db(tmp_path)

        send = _run_siftd("--db", str(source), "db", "send", "--tag", "public")
        assert send.returncode == 0
    ...
    def test_send_with_multi_tag_filter(self, tmp_path):
    ...
    def test_send_with_no_tag_filter(self, tmp_path):
        source = _seed_db(tmp_path)

        send = _run_siftd("--db", str(source), "db", "send", "--no-tag", "private")
        assert send.returncode == 0
    ...
    def test_send_with_multi_no_tag_filter(self, tmp_path):
    ...
    def test_send_with_since_filter(self, tmp_path):
        source = _seed_db(tmp_path)
    ...
    def test_send_combined_filters(self, tmp_path):
...
class TestSyncStatusCommand:
    ...
    def test_sync_status_output(self, tmp_path):
        db = tmp_path / "test.db"
        open_database(db).close()

        result = _run_siftd("--db", str(db), "db", "sync-status")
...
class TestStagedReceive:
    ...
    def test_stage_then_process(self, tmp_path):
        source = _seed_db(tmp_path)
        target = tmp_path / "target.db"
        ...
        status = _run_siftd("--db", str(target), "db", "sync-status")
        inbox = json.loads(status.stdout)["inbox"]
        assert inbox["pending"] == 1

        # Process the inbox
        process = _run_siftd("--db", str(target), "db", "process")
