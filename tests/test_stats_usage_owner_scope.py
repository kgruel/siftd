...
def _seed(db, *, with_owners: bool) -> None:
    ...
    conn.executescript(
        ...
        # usage_by_conv_model rollup (schema v10): the per-(conv,model) usage fact
        # the breakdowns now group over. Rows mirror conversation_stats so the
        # owner-scoped assertions below hold. Cache columns default to 0.
        ...
        "INSERT INTO usage_by_conv_model VALUES ('cA','mA',NULL,10,20,1,1,1.5,0,0);"
        # Bob's conversation
        ...
        "INSERT INTO usage_by_conv_model VALUES ('cB','mB',NULL,100,200,1,1,9.0,0,0);"
    )
    if with_owners:
        conn.executescript(
