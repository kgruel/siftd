---
name: strata-api
description: "Encode strata search patterns into reusable Python scripts or SQL queries. Use when a search pattern is worth automating, or when CLI exploration should graduate to programmatic access."
---

# Strata API — From Exploration to Encoding

Strata is API-first. The CLI exists for exploration; the API is where useful patterns become durable queries.
...
strata ask -w $(basename $PWD) "what we decided about authentication" --thread

# Find when a concept first appeared
strata ask --first "event sourcing"

# List recent conversations with a tag
strata query -l research:auth --since 2025-01

# Drill into a specific conversation
strata query 01KG0EX2X2A1
```
...
Use `strata.api` for programmatic access:

```python
#!/usr/bin/env python3
"""Find and tag conversations about a principle."""

from strata.api import hybrid_search, aggregate_by_conversation
...
Save to `~/.config/strata/scripts/` for reuse.

## Phase 4: Encode as SQL

For declarative queries, create `.sql` files in `~/.config/strata/queries/`:

```sql
-- ~/.config/strata/queries/auth-decisions.sql
...
Run with: `strata query sql auth-decisions workspace=myproject limit=10`
...
```python
from strata.api import hybrid_search, aggregate_by_conversation, first_mention
...
```python
from strata.api import list_conversations, get_conversation
...
```python
from strata.api import run_query_file, list_query_files
...
1. **Scripts go in** `~/.config/strata/scripts/` — not in project repos
2. **SQL queries go in** `~/.config/strata/queries/*.sql`
