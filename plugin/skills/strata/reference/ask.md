...
```bash
strata ask "token refresh" --context 3
```
...
```bash
strata ask --full "schema migration"
```
...
```bash
strata ask --refs "authelia setup"              # all file refs
strata ask --refs HANDOFF.md "setup"            # only refs to specific file
strata ask --refs HANDOFF.md,schema.sql "setup" # comma-separated file filter
```
...
```bash
strata ask --chrono "state management"
strata ask --chrono --since 2024-06 "state management"
```
Traces how a concept evolved across sessions.

**`--json`** — structured JSON output:
```bash
strata ask --json "error handling"
```
...
```bash
strata ask --format compact "error handling"
```
...
```bash
strata ask -w myproject "auth flow"
strata ask -w myproject --thread "auth flow"      # workspace + output mode
```
The single most impactful filter. Always use when you know the project.

**`-m` / `--model NAME`** — filter by model name:
```bash
strata ask -m claude-3-opus "architecture"
```

**`--since DATE` / `--before DATE`** — date range:
```bash
strata ask --since 2025-01 "migration"                    # after date
strata ask --since 2025-01 --before 2025-06 "migration"   # window
```
Dates are ISO format or YYYY-MM-DD.

**`-l` / `--tag NAME`** — filter by conversation tag (OR logic, repeatable):
```bash
strata ask -l research:auth "token expiry"                # single tag
strata ask -l research:auth -l research:security "tokens" # either tag (OR)
```
...
```bash
strata ask --threshold 0.7 "event sourcing"
```
Scores: 0.7+ on-topic, 0.6-0.7 tangential, <0.6 noise.

**`--role {user,assistant}`** — filter chunks by source role:
```bash
strata ask --role user "what should we do about"    # just human prompts
...
```bash
strata ask -n 20 "error handling"
```
...
```bash
strata ask --conversations "state management"
```
...
```bash
strata ask --embeddings-only "chunking strategy"
```
...
```bash
strata ask --no-exclude-active "current discussion"
```
...
```bash
strata ask --backend ollama "error handling"
```

**`--embed-db PATH`** — alternate embeddings database path:
```bash
strata ask --embed-db /path/to/alt.db "query"
```
...
strata ask -w myproject --thread "why we chose JWT"

# Trace evolution of an idea over time in one workspace
strata ask -w myproject --chrono --since 2024-06 "state management"

# High-relevance results only, with file references
strata ask --threshold 0.7 --refs "schema migration"

# Search tagged conversations with context
strata ask -l research:auth --context 2 "token rotation"

# Find earliest mention across all workspaces
strata ask --first --threshold 0.65 "event sourcing"

# Cross-workspace comparison
strata ask -w projectA "caching strategy"
strata ask -w projectB "caching strategy"

# Exclude archived conversations, narrative view
strata ask --no-tag archived --thread "authentication redesign"

# Role-filtered search within a date range
strata ask --role user --since 2025-01 "what should we do about"
```
