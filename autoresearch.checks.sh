...
if errors:
    ...
    for e in errors:
        print(f'  {e}')
    sys.exit(1)
" tests/test_serve_routes_edges.py tests/test_serve_routes_stepup.py

# Lint
uv run python -m ruff check tests/test_serve_routes_edges.py tests/test_serve_routes_stepup.py 2>&1 | tail -10
