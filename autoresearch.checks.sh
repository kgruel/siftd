...
if errors:
    ...
    for e in errors:
        print(f'  {e}')
    sys.exit(1)
" tests/test_tags_temporal.py tests/test_api_tags_edges.py

# Lint
uv run python -m ruff check tests/test_tags_temporal.py tests/test_api_tags_edges.py 2>&1 | tail -10
