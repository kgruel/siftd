...
def _reorder_subparsers(subparsers, order: list[str]) -> None:
    """Reorder subparser display order to keep CLI help output stable."""
    name_parser_map = subparsers._name_parser_map
    current_names = list(name_parser_map.keys())
    ordered_names = [name for name in order if name in name_parser_map]
    ordered_names.extend(name for name in current_names if name not in ordered_names)

    reordered_map = {name: name_parser_map[name] for name in ordered_names}
    name_parser_map.clear()
    name_parser_map.update(reordered_map)

    existing_actions = subparsers._choices_actions
    actions_by_dest = {action.dest: action for action in existing_actions}
    ordered_actions = [actions_by_dest[name] for name in ordered_names if name in actions_by_dest]
    ordered_actions.extend(action for action in existing_actions if action.dest not in ordered_names)
    subparsers._choices_actions = ordered_actions
...
NOTE: Session content may contain sensitive information (API keys, credentials, etc.).""",
  ...
  siftd export --last -o context.md     # write to file""",
    ...
    # Keep CLI help ordering stable after grouped subparser registration.
    _reorder_subparsers(
        subparsers,
        [
            "ingest",
