...
A fourth invariant governs reads: every read-only connection in siftd — here, in `embeddings.py`, and in `doctor/` — is opened by `connect_read_only()` in [sqlite.py](sqlite.py), which **derives** immutability from the medium instead of asserting it. Do not hand-roll a `mode=ro` URI. `immutable=1` tells SQLite the file cannot change and to skip all locking and change detection, which is a promise this codebase cannot make about its own database: a reader that asserted it silently answered from the last checkpoint whenever a writer held un-checkpointed commits, and reported corruption in healthy databases when a writer checkpointed mid-scan. The helper tries a plain `mode=ro` open, falls back to `immutable=1` only when the `-shm` sidecar cannot be created — a medium no writer can reach — and refuses even that when a `-wal` or hot `-journal` alongside the file holds state the fallback would silently drop, since `immutable=1` reads only the main file. That last check is deliberately conservative for the WAL and will occasionally refuse a database that would have read fine: whether a WAL's frames are already backfilled is recorded in the `-shm`, which is by definition the file that could not be created, so the bytes alone cannot distinguish a current database from one missing every commit. SQLite resolves the same ambiguity the same way. `tests/architecture/test_readonly_opens.py` enforces the routing rule directly: it enumerates every literal `mode=ro` or `immutable=1` URI under `src/siftd/` and fails on any that is not `connect_read_only` itself or the adapters-SDK carve-out, which stays hand-rolled so the adapter boundary holds. Backups route through the helper too — a backup is precisely the read that must not copy a stale snapshot.
