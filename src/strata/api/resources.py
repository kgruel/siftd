...
from strata.paths import adapters_dir, queries_dir
...
def copy_adapter(
    ...
    *,
    ...
    try:
        source_ref = importlib.resources.files("strata.adapters").joinpath(f"{name}.py")
    except (ModuleNotFoundError, TypeError) as e:
        raise CopyError(f"Cannot locate adapter package: {e}") from e

    if not source_ref.is_file():
        from strata.api.adapters import list_builtin_adapters
