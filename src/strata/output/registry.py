...
if TYPE_CHECKING:
    from strata.output.formatters import OutputFormatter
...
def load_dropin_formatters(path: Path) -> dict[str, ModuleType]:
    ...
    for py_file in sorted(path.glob("*.py")):
        if py_file.name.startswith("_"):
            continue
        module_name = f"strata_dropin_formatter_{py_file.stem}"
        try:
...
def load_entrypoint_formatters() -> dict[str, ModuleType]:
    """Discover formatters registered via the 'strata.formatters' entry point group."""
    formatters: dict[str, ModuleType] = {}
    eps = importlib.metadata.entry_points(group="strata.formatters")
...
class FormatterRegistry:
    ...
    def __init__(self, dropin_path: Path | None = None):
        from strata.paths import formatters_dir
