...
from tbd.paths import adapters_dir, queries_dir
...
def copy_adapter(
    ...
    *,
    ...
    try:
        source_ref = importlib.resources.files("tbd.adapters").joinpath(f"{name}.py")
    except (ModuleNotFoundError, TypeError) as e:
        raise CopyError(f"Cannot locate adapter package: {e}") from e

    if not source_ref.is_file():
        from tbd.api.adapters import list_builtin_adapters
...
def copy_query(
    ...
    *,
    ...
    try:
        ...
        # Fallback: try relative to tbd package
        try:
            source_ref = importlib.resources.files("tbd").joinpath(
                f"../../../queries/{name}.sql"
            )
