...
class CheckContext:
    ...
    def get_db_conn(self):
        ...
        if self._db_conn is None:
            from tbd.storage.sqlite import open_database
...
class EmbeddingsStaleCheck:
    ...
    def run(self, ctx: CheckContext) -> list[Finding]:
        ...
        from tbd.storage.embeddings import get_indexed_conversation_ids
...
class DropInsValidCheck:
    ...
    def _check_adapters(self, adapters_dir: Path) -> list[Finding]:
        ...
        from tbd.adapters.registry import _REQUIRED_ATTRS, _REQUIRED_CALLABLES, _VALID_DEDUP_STRATEGIES
        ...
        for py_file in sorted(adapters_dir.glob("*.py")):
            ...
            module_name = f"tbd_doctor_check_adapter_{py_file.stem}"
            try:
    ...
    def _check_formatters(self, formatters_dir: Path) -> list[Finding]:
        ...
        from tbd.output.registry import _REQUIRED_ATTRS, _REQUIRED_CALLABLES
        ...
        for py_file in sorted(formatters_dir.glob("*.py")):
            ...
            module_name = f"tbd_doctor_check_formatter_{py_file.stem}"
            try:
                spec = importlib.util.spec_from_file_location(module_name, py_file)
                if spec is None or spec.loader is None:
                    findings.append(
