...
```
siftd 0.10.2 - Aggregate and query LLM conversation logs
...
usage: siftd query [-h] [-w SUBSTR] [-m NAME] [--since DATE] [--before DATE]
                   ...
                   [--summary] [--neighbors] [conversation_id]
...
IDs shown are 12-char prefixes; any unambiguous prefix works. Pass an
...
```
siftd > search
Search conversations (auto-selects FTS5 or semantic based on what's installed)

usage: siftd search [-h] [-w SUBSTR] [-m NAME] [--since DATE] [--before DATE]
                    [-l NAME] [--all-tags NAME] [--no-tag NAME] [--on KIND]
                    [-t NAME] [--tool-tag NAME] [--owner USER] [--json]
                    [-n LIMIT] [-F] [-v] [--format NAME] [--around PHRASE]
                    [--turns A:B] [--select SELECTOR] [--sort ORDER]
                    [--view VIEW] [--refs [FILES]] [--mode ENGINE] [--recall N]
                    ...
                    [--backend NAME] [--embed-db PATH] [query ...]
...
Auto-selects the engine: hybrid (FTS5 + semantic) when embeddings are
...
usage: siftd export [-h] [-n [N]] [-w SUBSTR] [--since DATE] [--before DATE]
                    ...
                    [--no-header] [-o FILE] [conversation_id]
...
OPTIONS
  --models           Re-parse raw model names to canonical form (e.g.
                     claude-haiku-4.5 -> claude-haiku-4-5)
  --pricing          Reproject the pricing reference and rebuild the cost rollup
                     (run after editing pricing.toml)
...
```
siftd > copy
Copy built-in resources for customization

usage: siftd copy [-h] [--all] [--force] {adapter,query,formatter} [name]

ARGUMENTS
  {adapter,query,formatter}  Resource type to copy
  name                       Resource name
