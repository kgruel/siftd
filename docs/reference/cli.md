...
```
siftd 0.12.1 - Aggregate and query LLM conversation logs
...
usage: siftd doctor [-h] [--json] [--strict] [--pending-tags]
                    [--discard-unresolved] [--deep] [--fast] [--blob-refcount]
                    [--triggers] [--no-hints] [subcommand ...]
...
OPTIONS
  ...
  --pending-tags        Apply queued session tags to their ingested
                        conversations (use with 'fix')
  --discard-unresolved  Also delete pending tags that match no conversation (use
                        with 'fix --pending-tags'). Destructive.
