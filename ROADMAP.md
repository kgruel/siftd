...
- [x] Codex CLI adapter
- [x] Adapter plugin system (drop-in `~/.config/siftd/adapters/*.py` + entry points)
- [x] `siftd ask` — semantic search via embeddings (fastembed backend)
- [x] `siftd ask` tuning — exchange-window chunking, model evaluation (bge-small wins)
- [x] Unified chunking: production `siftd ask` and bench share `src/embeddings/chunker.py`
...
- [x] Installable package (`pyproject.toml`, `uv.lock`, `siftd.search.hybrid_search` public API)
...
- [ ] `siftd enrich` — auto-labeling via LLM, only for ops that justify the cost
...
| `siftd enrich` | Only justified for expensive operations (LLM classification). Manual labels cover current needs. |
