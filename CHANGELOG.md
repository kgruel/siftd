...
- **`ambiguous-id` caveat producer** — Emits a warning when a `siftd query <id>` detail view was retrieved using an ID prefix matching multiple conversations. Fires on detail renders of `get_conversation` when the queried ID is shorter than 26 characters (full ULID length) and matches more than one conversation in the database. Message includes the queried prefix, match count, and suggests using a longer prefix to disambiguate. Severity=warning; channel=both.
